...
def main():
    objc_data_seg = idc.SegByName('__objc_data')
    ...
    while ea < idc.SegEnd(objc_data_seg):
        ...
        objc_class.dump()
        '''
        class_struct = parse_class_struct(ea)
        if class_struct.get('class_def'):
            class_def = parse_class_def(class_struct['class_def'])
        else:
            print 'No class definition found'
            continue
            
        if class_def.get('method_struct'):
            parse_method_struct(class_def['method_struct'])

        if class_def.get('meta_protocols'):
            parse_protocols_list(class_def['meta_protocols'])

        if class_def.get('ivars_struct'):
            parse_ivars_struct(class_def['ivars_struct'])
            
        if class_def.get('properties'):
            parse_properties(class_def['properties'])
'''
        ea = ea + 0x14
#        if ea > objc_data_seg + 0x15:
#            break
